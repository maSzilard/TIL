function updateTPostQty() {
  ...
  updatePostQty(meshHeight, postQty);
...
function updatePegsQty() {
  ...
  const quantity = Number($("#addToCart .variations_form.cart .quantity .input-text").val());
  //calculate pegs
  const pegQty = meshLength * quantity;
  
  $('.woobt-item-product[data-key="slph"] input[type="number"]').val(pegQty);
  $('.woobt-item-product[data-key="slph"]').attr('data-qty', pegQty);
}
...
function updateCableTieQty() {
  ...
  $('.woobt-item-product[data-key="rgil"] input[type="number"]').val(CableTieQty);
  $('.woobt-item-product[data-key="rgil"]').attr('data-qty', CableTieQty);
}
...
function updateTensioningWireQty() {
  ...
  //qty
  const quantity = Number($("#addToCart .variations_form.cart .quantity .input-text").val());
  //calculate tensioning wire
  let tensioningWireQty;
  if (meshHeight >= 100) {
    tensioningWireQty = Math.ceil((meshLength * 3.1 * quantity) / 52);
  } else {
    tensioningWireQty = Math.ceil((meshLength * 2.1 * quantity) / 52);
  }

  $('.woobt-item-product[data-key="6vhr"] input[type="number"]').val(tensioningWireQty);
  $('.woobt-item-product[data-key="6vhr"]').attr('data-qty', tensioningWireQty);
}
...
function updateThisProduct() {
  ...
  $('.woobt-item-product[data-key="rgil"]').attr('data-qty', getProductQty);
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // bulk info elements
  $('.bulk-info-table').eq(0).before('<div class="bulk-info"><img width="18px" height="18px" src="https://env-wirefence-wfpremstg.kinsta.cloud/wp-content/uploads/2024/03/ic_baseline-local-offer.svg"> Bulk Discount & Save upto 15% <img width="18px" height="18px" src="https://env-wirefence-wfpremstg.kinsta.cloud/wp-content/uploads/2024/03/mingcute_down-line.svg"> </div>');
  ...
  $('.woobt-products div[data-key="6vhr"] .woobt-title-inner').append(createInfoWrapper('tensioning-wire-info', tensioningWireContent));
